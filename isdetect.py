...
def get_longest_text(assam_list,inst_kind,key_inst,key_inst_per):
	...
	while True:
		try:
			...
			break
	print("text_section:[%d,%d] text_per:%d%% inst_kinds:"%(result_start*4,result_end*4,100*(result_end-result_start)/len(assam_list)),len(set(assam_list[result_start:result_end])))
...
ARM_LITTLE_TEXT=get_longest_text(ARM_LITTLE_LIST,20,"ldr",5)
...
ARM_BIG_TEXT=get_longest_text(ARM_BIG_LIST,20,"ldr",5)
...
PPC_BIG_TEXT=get_longest_text(PPC_BIG_LIST,20,"lwz",4)
if judge_PPC(PPC_BIG_TEXT)==mat(([1])):
	observe_inst_per(PPC_BIG_TEXT)
	print("	result: PPC-Big")
	sys.exit(0)
...
PPC_LITTLE_TEXT=get_longest_text(PPC_LITTLE_LIST,20,"lwz",4)
if judge_PPC(PPC_LITTLE_TEXT)==mat(([1])):
	observe_inst_per(PPC_LITTLE_TEXT)
	print("	result: PPC-Little")
	sys.exit(0)
